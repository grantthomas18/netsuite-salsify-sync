...
def build_csv(items, prices, inv_us, inv_uk):
    ...
    for item in items:
        ...
        writer.writerow({
            ...
            "Wholesale Price (Netsuite)":           f"{float(p['wholesale']):.2f}" if p.get("wholesale") not in (None, "") else "",
            "MSRP Price (Netsuite)":               f"{float(p['msrp']):.2f}"       if p.get("msrp")       not in (None, "") else "",
            "Amazon Price (Netsuite)":             f"{float(p['amazon']):.2f}"     if p.get("amazon")     not in (None, "") else "",
            ...
            "Inventory Available US (Netsuite)":   int(us["qty_available"]) if us.get("qty_available") not in (None, "") else 0,
            "Inventory On Order US (Netsuite)":    int(us["qty_on_order"])   if us.get("qty_on_order")   not in (None, "") else "",
            "Inventory Available UK (Netsuite)":   int(uk["qty_available"]) if uk.get("qty_available") not in (None, "") else 0,
